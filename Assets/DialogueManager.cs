...
public class DialogueManager : MonoBehaviour
{
    ...
    private IEnumerator TurnCameraTowardsNPC(Transform NPC)
    {
        ...
        Quaternion targetRelation = Quaternion.LookRotation(NPC.position - playerCamera.position);

        float elapsedTime = 0f;
        while (elapsedTime < 1f)
        {
            playerCamera.rotation = Quaternion.Slerp(startRotation, targetRelation, elapsedTime);
            elapsedTime += Time.deltaTime * turnSpeed;
            yield return null;
        }

        playerCamera.rotation = targetRelation;
    }
