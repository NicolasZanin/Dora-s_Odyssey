...
public class DialogueManager : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        playerCamera = Camera.allCameras[0].transform;
    }
    ...
    {
        while (currentDialogueIndex < dialogList.Count)
        {
            DialogueString line = dialogList[currentDialogueIndex];
    ...
    private IEnumerator TypeText(string text)
    {
        ...
        if (!dialogList[currentDialogueIndex].isQuestion)
        {
            yield return new WaitUntil(() => Input.GetMouseButtonDown(0));
        }

        if (dialogList[currentDialogueIndex].isEnd)
            DialogueStop();
        else if (dialogList[currentDialogueIndex].beginCourse)
            BeginCourse();
